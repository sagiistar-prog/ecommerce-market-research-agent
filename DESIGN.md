...
Operate 模式沿用既有双栏，增加 Hypotheses 分区。逐条假设用分隔线组织，标题18px复用既有title尺寸；详细推理、假设和测试计划折叠，引用可展开原始字段。选择和理由紧邻 Record choice，记录后回到本条选择并保留可见焦点。导入、下载、撤销使用既有辅助按钮。

默认未评审，记录理由必填，输入过期时禁用评审操作。失败保留上次有效评审；切换视图保留未记录草稿，导出前要求记录。重开页面通过显式导入恢复，不能把 Record choice 写成已持久保存。

本轮 Impeccable 机械扫描提示字体尺寸与配置表不完全一致；新标题复用18px，既有14、21、26px保留于各自辅助、分区、移动标题用途。未借此替换既有视觉体系。桌面和手机验收包含实际任务与截图；自动无障碍检测通过不等于完整认证。

## 0.5 成对任务测试

Tests 沿用 Operate 模式，宽屏计划表单双栏，560px 以下单列。字段、按钮和下拉最小高度44px。结果先展示判定、样本量、成对改善和护栏分母，再折叠保留计划与采集方法。测试视图隐藏原分析报告的导出条，避免把报告下载误认为测试结果下载。失败保留已录入计划和上次有效结果，材料变化后禁止导出。只采用原生控件与既有蓝灰色，没有新增装饰和远程资产。

机械扫描运行一次，HTML/CSS 解析缺失时降级；发现仍为既有字体/字号配置差异，未当作新的设计缺陷或全量通过。两轮内检查桌面和手机截图，先发现结果被计划说明挤出视野，再将结果前置。
