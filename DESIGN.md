...
规则依据 web/styles.css、web/index.html 与 web/app.js。界面中的离线模板说明和虚构数据性质是持续约束，不是可删除的多余注释。
...
正文使用本机无衬线栈；Inter 未在此样式中自托管。主标题在 560px 以下变为 26px；报告标题为 28px，正文行高 1.58。竞品 CSV 输入保留 Cascadia Mono / Consolas 回退，字号为 13px，区别于普通输入。
...
两块主工作面使用细边框与相同柔和阴影（0 16px 40px rgba(31, 41, 51, 0.12)），内部依靠分隔线与留白。按钮背景与阴影过渡为 160ms ease-out，按下位移 1px；reduced-motion 关闭过渡和动画。
...
- Primary 和 secondary 按钮的类规则最小高度为 40px；其优先级高于通用 button 的 44px 声明。不能将后者当作实际统一尺寸。
